...
def ENHANCED_CLUSTER(FUNC_PATH, FORMULA):
...
def CREATE_NESTED_OUTPUT(EXTRACTOR_DICT, FUNC_PATH, FORMULA):
    def STANDARDIZE_CELL_REFERENCE(CELL_REF):
        DOM_PATTERN                             = r'([A-Za-z0-9_]+!)?([A-Za-z]+)([0-9]+)'
        CELL_PATTERN                            = r"^[A-Z]{1,2}\d+$"  
        ARRAY_PATTERN                           = r"^\{.*\}$" 
        if "!" in CELL_REF:                     _, STRIPPED = CELL_REF.split("!", 1)
        else:                                   STRIPPED = CELL_REF
        if re.match(CELL_PATTERN, STRIPPED):    STANDARDIZED_REF = re.sub(DOM_PATTERN, r'\1\2*', CELL_REF)
        else:                                   STANDARDIZED_REF = CELL_REF
    ...
    CLUSTERING_INFO = ENHANCED_CLUSTER(FUNC_PATH, FORMULA)
...
def FULL_RUN(FILE_PATH, FUNC_PATH, LABELS):
    ...
    NESTED_OUTPUT           = CREATE_NESTED_OUTPUT(EXTRACTOR.DICT, FUNC_PATH, FORMULA)
...
def TEST_RUN(FILE_PATH, FUNC_PATH, LABELS):
    ...
    return PARSED_DATA, EXTRACTOR.DICT, P3_RESULTS, FUNC_PATH, FORMULA
